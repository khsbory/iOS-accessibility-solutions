...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    ["웹뷰 데모", "접근성 포커스 데모", "페이지 전환 데모", "필터 데모", "뮤직플레이어", "드래그 & 드롭", "롤링배너", "캐러셀 접근성", "설정", "화면스크롤","분리된 접근성 초점 개선","밀기 동작에 대한 접근성 데모","접근성 초점 재조정","보이스오버 스크롤 테스트","롤링배너 2탄","테이블뷰 순서이동 데모","테이블뷰 리로드 데모","UISlider 뮤직 플레이어 데모","캘린더 데모","커스텀 로터 데모","HeadingLevel, accessibilityLanguage 테스트"]
 ...
 if (indexPath.row == 0) {
            ...
            showScreenOnOtherStoryboard(storyboardName: "UISliderMusicPlayerDemo", viewControllerStoryboardId: "UISliderMusicPlayerMain")
        } else if (indexPath.row == 18) {
            showScreenOnOtherStoryboard(storyboardName: "CalendarDemo", viewControllerStoryboardId: "CalendarMain")
        } else if (indexPath.row == 19) {
            showScreenOnOtherStoryboard(storyboardName: "CustomRotor", viewControllerStoryboardId: "CustomRotorMain")
        } else if (indexPath.row == 20) {
            showScreenOnOtherStoryboard(storyboardName: "HeadingLevelAndLanguage", viewControllerStoryboardId: "HeadingLevelAndLanguageMain")
        }
